...
void callbackDispatcher() {
  Workmanager().executeTask((taskName, inputData) async {
    ...
    await notifications.initialize(initSettings); // No onDidReceive... callbacks here for background
    ...
    const AndroidNotificationChannel channel = AndroidNotificationChannel(
      ...
      description: 'Provides a daily summary of your expenses.', // User-visible description
    ...
    if (taskName == "expenseSummaryTask") {
      ...
      if (todayItems.isNotEmpty) {
        ...
        double minAmount = todayItems.map((e) => e.amount).reduce((a, b) => a < b ? a : b);
        double maxAmount = todayItems.map((e) => e.amount).reduce((a, b) => a > b ? a : b);
        ...
        await notifications.show(
          ...
          NotificationDetails( // Use the defined channel
            android: AndroidNotificationDetails(
              ...
              priority: Priority.high, // Set priority here for the specific notification
              icon: '@mipmap/ic_launcher', // Ensure icon is set for background notifications
...
void notificationTapBackground(NotificationResponse notificationResponse) {
  debugPrint('Notification tapped in background (terminated app): ${notificationResponse.payload}');
...
void main() async {
  ...
  final FlutterLocalNotificationsPlugin foregroundNotifications = FlutterLocalNotificationsPlugin();
  const AndroidInitializationSettings foregroundAndroidInit = AndroidInitializationSettings('@mipmap/ic_launcher');
  const InitializationSettings foregroundInitSettings = InitializationSettings(android: foregroundAndroidInit);
  ...
  await foregroundNotifications.initialize(
    ...
    onDidReceiveBackgroundNotificationResponse: notificationTapBackground, // Corrected: Pass the dedicated background tap handler
  );
  ...
  Workmanager().registerPeriodicTask(
    ...
    "expenseSummaryTask",
    frequency: const Duration(hours: 1), // Run once every 24 hours
    initialDelay: const Duration(seconds: 30),
    constraints: Constraints(
      networkType: NetworkType.not_required,
      requiresBatteryNotLow: false, // Set to true if you want to delay execution when battery is low
